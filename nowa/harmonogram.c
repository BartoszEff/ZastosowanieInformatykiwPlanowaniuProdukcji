...
static void generuj_sasiedztwo(int *kolejnosc_zadan, int liczba_zadan, int *sasiednie_rozwiazanie) {
...
EXPORT void generuj_dane(int liczba_zadan, int liczba_maszyn,int *czasy_przetwarzania,int *typy_zadan,int *czasy_przezbrojen) {
...
EXPORT int oblicz_makespan(int *kolejnosc_zadan,int liczba_zadan,int liczba_maszyn,int *czasy_przetwarzania,int *typy_zadan,int *czasy_przezbrojen) {
...
EXPORT void symulowane_wyzarzanie(int *kolejnosc_zadan,int liczba_zadan,int liczba_maszyn,int *czasy_przetwarzania,int *typy_zadan,int *czasy_przezbrojen,int *najlepsze_rozwiazanie,int *wynik_makespan)
{
    ...
    *wynik_makespan = najlepszy_makespan;
}

EXPORT void symulowane_wyzarzanie_param(int *kolejnosc_zadan,int liczba_zadan,int liczba_maszyn,int *czasy_przetwarzania,int *typy_zadan,int *czasy_przezbrojen,int *najlepsze_rozwiazanie,int *wynik_makespan,double temperatura,double wspolczynnik_chlodzenia,double minimalna_temperatura,int maks_iteracji,int brak_poprawy,int maks_brak_poprawy)
{
    int aktualny_makespan = oblicz_makespan(kolejnosc_zadan,liczba_zadan,liczba_maszyn,czasy_przetwarzania,typy_zadan,czasy_przezbrojen);
    int najlepszy_makespan = aktualny_makespan;
    memcpy(najlepsze_rozwiazanie, kolejnosc_zadan, sizeof(int) * liczba_zadan);
...
static void brute_force_rekurencja(int *kolejnosc_zadan,int start,int liczba_zadan,int liczba_maszyn,int *czasy_przetwarzania,int *typy_zadan,int *czasy_przezbrojen,int *najlepszy_makespan,int *najlepsze_rozwiazanie)
{
    ...
    for (int i = start; i < liczba_zadan; i++) {
        ...
        brute_force_rekurencja(kolejnosc_zadan, start + 1, liczba_zadan, liczba_maszyn,czasy_przetwarzania, typy_zadan,czasy_przezbrojen,najlepszy_makespan, najlepsze_rozwiazanie);
...
EXPORT void brute_force(int *kolejnosc_zadan,int start,int liczba_zadan,int liczba_maszyn,int *czasy_przetwarzania,int *typy_zadan,int *czasy_przezbrojen,int *najlepszy_makespan,int *najlepsze_rozwiazanie)
{
    ...
    *najlepszy_makespan = INT_MAX;
    int *aktualna = (int *)malloc(sizeof(int) * liczba_zadan);
    memcpy(aktualna, kolejnosc_zadan, sizeof(int) * liczba_zadan);

    brute_force_rekurencja(aktualna, start, liczba_zadan, liczba_maszyn,czasy_przetwarzania, typy_zadan, czasy_przezbrojen, najlepszy_makespan, najlepsze_rozwiazanie);

    free(aktualna);
}

EXPORT int generuj_dane_harmonogramu(int *kolejnosc_zadan,int liczba_zadan,int liczba_maszyn,int *czasy_przetwarzania,int *typy_zadan,int *czasy_przezbrojen, Operacja *operacje)
{
    int *czasy_zakonczenia_maszyn = (int *)malloc(sizeof(int) * liczba_maszyn);
    memset(czasy_zakonczenia_maszyn, 0, sizeof(int) * liczba_maszyn);
    ...
    for (int m = 0; m < liczba_maszyn; m++) {
        for (int i = 0; i < liczba_zadan; i++) {
            ...
            int koniec_poprzedniej_maszyny =
                (m > 0) ? czasy_zakonczenia_zadan[zadanie * liczba_maszyn + (m - 1)] : 0;
