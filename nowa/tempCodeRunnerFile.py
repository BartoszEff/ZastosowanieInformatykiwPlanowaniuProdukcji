...
def accept(self):
        ...
        if minVal >= tempVal:
            QMessageBox.warning(
                self,
                "Błąd parametrów",
                "Minimalna temperatura nie może być większa lub równa temperaturze początkowej."
            )
            return 
...
class ParamSA_Dialog(QDialog):
    def __init__(self):
        ...
        self.edTemp.editingFinished.connect(lambda: validate_number_input(self.edTemp, self.edTemp.text(), 100.0, 1e9))
        self.edChlodz = QLineEdit("0.95")
        self.edChlodz.editingFinished.connect(lambda: validate_number_input_float(self.edChlodz, self.edChlodz.text(), 0.01, 0.9999))
        self.edMinTemp = QLineEdit("0.05")
        self.edMinTemp.editingFinished.connect(lambda: validate_number_input_float(self.edMinTemp, self.edMinTemp.text(), 0.01, 999999))
        self.edMaxIter = QLineEdit("1000")
        self.edMaxIter.editingFinished.connect(lambda: validate_number_input(self.edMaxIter, self.edMaxIter.text(), 500, 9999999))
        self.edBrakPop = QLineEdit("0")
        self.edBrakPop.editingFinished.connect(lambda: validate_number_input(self.edBrakPop, self.edBrakPop.text(), 0, 9999999))
        self.edMaxBrak = QLineEdit("500")
        self.edMaxBrak.editingFinished.connect(lambda: validate_number_input(self.edMaxBrak, self.edMaxBrak.text(), 1, 9999999))
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.edZad.editingFinished.connect(lambda: validate_number_input(self.edZad, self.edZad.text(), 1, 30))
        row.addWidget(self.edZad)

        row.addWidget(QLabel("Maszyny:"))
        self.edMasz = QLineEdit("3")
        self.edMasz.editingFinished.connect(lambda: validate_number_input(self.edMasz, self.edMasz.text(), 1, 30))
    ...
    def akcja_csv(self):
        ...
        # Wczytywanie typów zadań, jeśli istnieją
        ...
        # Wczytywanie czasów przezbrojenia, jeśli istnieją
