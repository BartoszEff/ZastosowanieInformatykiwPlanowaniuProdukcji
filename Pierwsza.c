...
int oblicz_makespan(int kolejnosc_zadan[], int liczba_zadan, int liczba_maszyn,
                    int czasy_przetwarzania[liczba_zadan][liczba_maszyn],
                    int typy_zadan[liczba_zadan], int czasy_przezbrojen[liczba_maszyn]) {
    int czasy_zakonczenia_maszyn[liczba_maszyn];
    memset(czasy_zakonczenia_maszyn, 0, liczba_maszyn * sizeof(int));
    int czas_zakonczenia_przezbrojenia_globalny = 0;
    int makespan = 0;
    ...
    for (int i = 0; i < liczba_zadan; i++) {
        ...
        for (int m = 0; m < liczba_maszyn; m++) {
            ...
            int start_przezbrojenia = fmax(czasy_zakonczenia_maszyn[m], czas_zakonczenia_przezbrojenia_globalny);
            int koniec_przezbrojenia = start_przezbrojenia + czas_przezbrojenia;
            czas_zakonczenia_przezbrojenia_globalny = koniec_przezbrojenia;
...
void wyswietl_harmonogram_maszyn(int kolejnosc_zadan[], int liczba_zadan, int liczba_maszyn, int czasy_przetwarzania[liczba_zadan][liczba_maszyn], int typy_zadan[liczba_zadan], int czasy_przezbrojen[liczba_maszyn]) {
    ...
    for (int m = 0; m < liczba_maszyn; m++) {
        ...
        for (int i = 0; i < liczba_zadan; i++) {
            ...
            printf("[Z%d: %d-%d (Przezbrojenie: %d-%d)] ", zadanie + 1, start_zadania, koniec_zadania, start_przezbrojenia, koniec_przezbrojenia);
        }
        printf("\n");
    }
}
